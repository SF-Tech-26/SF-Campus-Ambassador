// client/src/pages/Responsibility.jsx
...
const TimelineContainer = ({ fanAngle, isVisible, stepsRefs, updateCurve, handleScroll, getMarkerCenter }) => {
    ...
    // Pass refs out to the main component for external reference
    useEffect(() => {
...
const Responsibility = () => {
    ...
    const handleScroll = useCallback((timelineRef, curvePathRef, currentIsVisible) => {
        ...
        // --- FIXED: Path Reveal Calculation (Faster scroll effect) ---
        const timelineRect = timeline.getBoundingClientRect();
        const totalLength = curve.getTotalLength();


        const startDrawY = timelineRect.top + scrollY - window.innerHeight;


        const effectiveTimelineHeight = timelineRect.height;
        const scrollRange = effectiveTimelineHeight + (window.innerHeight * 0.25);


        const scrollDistance = scrollY - startDrawY;


        const pathProgress = Math.min(1, Math.max(0, scrollDistance / scrollRange));
        ...
        for (let i = 0; i < stepsCount; ++i) {
            const revealThreshold = i / (stepsCount > 1 ? stepsCount - 1 : 1);
            const reveal = pathProgress >= revealThreshold;

            if (reveal !== newIsVisible[i]) { newIsVisible[i] = reveal; changed = true; }
        }
        if (changed) setIsVisible(newIsVisible);
    }, [popRandomEmoji, stepsRefs]);
    ...
    return (
        <div
            id="responsibility"
            className="sans bg-cover bg-center bg-black text-slate-50 min-h-screen  overflow-x-hidden overflow-y-hidden bg-no-repeat relative"
        ...
        >
            <div className="content-wrap relative z-10">
                ...
                <TimelineContainer
                    ...
                    handleScroll={(tRef, cRef) => handleScroll(tRef, cRef, isVisible)}
